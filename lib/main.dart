...
import 'widget_config/widget_data.dart';
import 'widget_config/config_page.dart';
...
class _StarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final star in stars) {
      ...
      // 透明度下?.2，保证星点不会完全消?
      final alpha = ((0.2 + 0.8 * (sin(twinkle * pi + star.phase) + 1) / 2) *
              star.brightness)
          .clamp(0.0, 1.0);
...
<style>html,body,#map{margin:0;padding:0;width:100%;height:100%}.amap-logo,.amap-copyright{display:none!important}${Platform.isIOS ? '.amap-maps{filter:invert(1) hue-rotate(180deg) brightness(0.9) contrast(0.85)}' : ''}</style>
...
class _HomePageState extends State<HomePage>
  ...
  static const MethodChannel _widgetChannel =
      MethodChannel('com.myapp.my_eado/widget');
    ...
    if (resp.code == 3) {
      ...
      try {
        ...
        if (presetEnabled && presetPin.length == 6) {
          resp = await _api.executeCommand(cmd,
              extraParams: extraParams, pin: presetPin);
          if (resp.code == 0) {
            return resp;
          }
          // 预设码错误，继续弹出手动输入
        ...
        resp =
            await _api.executeCommand(cmd, extraParams: extraParams, pin: pin);
        return resp;
      } finally {
  ...
  void initState() {
    ...
    _loadData();
    _startPolling();
    // 注册WorkManager定时更新小组件数据（APP关闭后仍可后台运行）
    registerWidgetPeriodicTask();
    // 注册热启动action监听（必须在拉取初始action之前）
    _widgetChannel.setMethodCallHandler(_onWidgetChannelCall);
    _checkDeepLink();
  }

  /// 热启动：原生主动推送的小组件action
  Future<dynamic> _onWidgetChannelCall(MethodCall call) async {
    if (call.method == 'onWidgetAction') {
      _dispatchWidgetAction(call.arguments as String?);
    }
    return null;
  }
  ...
  Future<void> _checkDeepLink() async {
    ...
    _dispatchWidgetAction(action);
  }

  void _dispatchWidgetAction(String? action) {
    if (action == null || !mounted) return;
    if (action == 'widget_config' || action.startsWith('widget_configure:')) {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const WidgetConfigPage()),
      );
    } else if (action.startsWith('widget_control:')) {
      final command = action.substring('widget_control:'.length);
      _handleWidgetCommand(command);
    }
  }

  void _handleWidgetCommand(String command) {
    switch (command) {
      case 'ctrl_lock':
        _sendCtrlCommand('lock');
        break;
      case 'ctrl_unlock':
        _sendCtrlCommand('unlock');
        break;
      case 'ctrl_ignition_on':
        _sendCtrlCommand('ignition_on');
        break;
      case 'ctrl_ignition_off':
        _sendCtrlCommand('ignition_off');
        break;
      case 'ctrl_window_open':
        _sendCtrlCommand('window_open');
        break;
      case 'ctrl_window_half':
        _sendCtrlCommand('window_half');
        break;
      case 'ctrl_window_close':
        _sendCtrlCommand('window_close');
        break;
      case 'ctrl_sunroof_open':
        _sendCtrlCommand('sunroof_open');
        break;
      case 'ctrl_sunroof_tilt':
        _sendCtrlCommand('sunroof_tilt');
        break;
      case 'ctrl_sunroof_close':
        _sendCtrlCommand('sunroof_close');
        break;
      case 'ctrl_ac_on':
        _sendCtrlCommand('ac_on');
        break;
      case 'ctrl_ac_off':
        _sendCtrlCommand('ac_off');
        break;
      default:
        break;
    }
  ...
  static const Map<String, String> _ctrlLabels = {
    'lock': '锁车',
    'unlock': '解锁',
    'ignition_on': '点火',
    'ignition_off': '熄火',
  ...
  // 小组件控制指令 → 车控平台命令
  static const Map<String, String> _ctrlCommands = {
    'lock': 'LockDoor',
    'unlock': 'UnLockDoor',
    'ignition_on': 'OpenEngine',
    'ignition_off': 'CloseEngine',
    'window_open': 'OpenWindow',
    'window_half': 'WindowSlit',
    'window_close': 'CloseWindow',
    'sunroof_open': 'SkyWindowLiftUp',
    'sunroof_tilt': 'SkyWindowLiftUp',
    'sunroof_close': 'CloseDormer',
  };

  /// 显示2秒悬浮提示（Toast风格，无交互弹窗）
  void _showFloatingToast(String message) {
    if (!mounted) return;
    _floatingToast?.remove();
    final entry = OverlayEntry(
      builder: (_) => _FloatingToastView(
        message: message,
        onFinished: () {
          if (_floatingToast?.mounted ?? false) _floatingToast!.remove();
          _floatingToast = null;
        },
      ),
    );
    _floatingToast = entry;
    Overlay.of(context).insert(entry);
  }

  Future<void> _sendCtrlCommand(String cmd) async {
    final name = _ctrlLabels[cmd] ?? cmd;

    // 空调走空调接口
    if (cmd == 'ac_on' || cmd == 'ac_off') {
      final api = ApiService();
      final r =
          await api.executeACCommand(cmd == 'ac_on' ? 'openAir' : 'closeAir');
      _showFloatingToast(r.code == 0 ? '$name指令已发送' : (r.msg ?? '$name指令失败'));
      return;
    }

    // 其他车控指令走统一控车接口（含PIN校验）
    final platformCmd = _ctrlCommands[cmd];
    if (platformCmd == null) return;
    if (_commandSending) return;
    _commandSending = true;
    _showFloatingToast('$name指令发送中...');
    try {
      final resp = await _executeWithPinCheck(platformCmd);
      if (!mounted) return;
      if (resp.code == 0) {
        _showFloatingToast('$name成功');
        _refreshCarData();
      } else {
        _showFloatingToast('$name失败: ${resp.msg}');
      }
    } catch (e) {
      if (mounted) _showFloatingToast('$name失败');
    } finally {
  ...
  String _lightStatus() {
    ...
    return lights.any((v) => v == 1) ? '已开' : '已关';
  }

  String _powerBatteryStatus() {
    final d = _carData ?? {};
    return (d['powerBatteryStatus'] == 0) ? '正常' : '异常';
  }

  String _avgFuel() {
    final d = _carData ?? {};
    final v = d['fuelConsumption100km'];
    return v != null ? '$v L/100km' : '--';
  }

  String _totalFuel() {
    final d = _carData ?? {};
    final v = d['igniteCumulativeOil'];
    return v != null ? '$v L' : '--';
  }

  String _waterTemp() {
    final d = _carData ?? {};
    final v = d['engineWaterTemp'];
    return v != null ? '$v°C' : '--';
  }
  ...
  String _tirePressureFL() {
    final d = _carData ?? {};
    final v = d['lfTyrePressure'] ??
        d['leftFrontTirePressure'] ??
        d['tirePressureFL'];
    return v != null ? '$v' : '--';
  }

  String _tirePressureFR() {
    final d = _carData ?? {};
    final v = d['rfTyrePressure'] ??
        d['rightFrontTirePressure'] ??
        d['tirePressureFR'];
    return v != null ? '$v' : '--';
  }

  String _tirePressureRL() {
    final d = _carData ?? {};
    final v =
        d['lrTyrePressure'] ?? d['leftRearTirePressure'] ?? d['tirePressureRL'];
    return v != null ? '$v' : '--';
  }

  String _tirePressureRR() {
    final d = _carData ?? {};
    final v = d['rrTyrePressure'] ??
        d['rightRearTirePressure'] ??
        d['tirePressureRR'];
    return v != null ? '$v' : '--';
  }
  ...
  void _updateWidgetData() {
    ...
    try {
      ...
      final unlocked = locks.any((e) => e == 0 || e == '0');
      lock = unlocked ? '已解锁' : '已上锁';
      final doors = [
      ...
      door = doors.any((e) => e == 1 || e == '1') ? '有开启' : '已关闭';
      final wins = [
        d['diverWindow'],
        d['passengerWindow'],
        d['leftRearWindow'],
      ...
      win = wins.any((e) => e == 1 || e == '1') ? '有开启' : '已关闭';
      sun = (d['skyWindow'] == 1 || d['skyWindow'] == '1') ? '有开启' : '已关闭';
      trunk = (d['trunk'] == 1 || d['trunk'] == '1') ? '已开启' : '已关闭';
      engine = (d['fireStatus'] == 2 || d['fireStatus'] == '2') ? '运行中' : '熄火';
      final bv = d['batteryVoltage'];
      batt = bv == null ? '--' : '${bv}V';
    } catch (_) {}
    WidgetData.updateVehicleData(
      acOn: _acOn,
      acTemp: _vehicleTemp,
      vehicleTemp: _vehicleTemp,
      envTemp: _envTemp,
      carPlate: _plateNumber.isNotEmpty ? _plateNumber : 'MyEADO',
      carName: _carName.isNotEmpty ? _carName : '未知车辆',
      range: _range,
      mileage: _totalOdometer,
      fuel: _fuelPercent,
      lockStatus: lock,
      doorStatus: door,
      windowStatus: win,
      ...
      powerBatteryStatus: _powerBatteryStatus(),
      avgFuel: _avgFuel(),
      totalFuel: _totalFuel(),
      waterTemp: _waterTemp(),
      voltage: batt,
      altitude: _altitude(),
      tirePressureFL: _tirePressureFL(),
      tirePressureFR: _tirePressureFR(),
      tirePressureRL: _tirePressureRL(),
      tirePressureRR: _tirePressureRR(),
    );
  }
...
<style>
  ...
  ${Platform.isIOS ? '.amap-maps { filter: invert(1) hue-rotate(180deg) brightness(0.9) contrast(0.85); }' : ''}
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  if (presetPinEnabled) {
                                    await prefs.setBool(
                                        'preset_pin_enabled', false);
                                    setState(() => presetPinEnabled = false);
                                  } else {
                                    if (presetPinController.text.length == 6) {
                                      await prefs.setBool(
                                          'preset_pin_enabled', true);
                                      await prefs.setString('preset_pin_value',
                                          presetPinController.text);
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            Navigator.pop(ctx);
                            final resp = await _api.updatePinStatus(false,
                                authCode: code);
                            if (resp.code == 0) {
...
/// 小组件控制指令的2秒悬浮提示（Toast风格：无交互、自动淡入淡出）
class _FloatingToastView extends StatefulWidget {
  final String message;
  final VoidCallback onFinished;

  const _FloatingToastView({required this.message, required this.onFinished});

  @override
  State<_FloatingToastView> createState() => _FloatingToastViewState();
}

class _FloatingToastViewState extends State<_FloatingToastView>
    with SingleTickerProviderStateMixin {
